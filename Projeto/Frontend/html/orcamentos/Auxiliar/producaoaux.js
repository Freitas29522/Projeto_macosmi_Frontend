(() => {
  const api = window.APP_CONFIG.API_BASE_URL + window.APP_CONFIG.API_PATH + "/orcamento-producao";
  ...
  const mKey = { Jan:"jan", Fev:"fev", Mar:"mar", Abr:"abr", Mai:"mai", Jun:"jun", Jul:"jul", Ago:"ago", Set:"sete", Out:"outu", Nov:"nov", Dez:"dez" };

  function markTouched(){ touched = true; setSaveEnabled(true); }

  // ===== Helpers de mapeamento
  const getMesVal = (obj, base, mes) => obj[ (mKey[mes] + base).toLowerCase() ];
  const setMesVal = (obj, base, mes, v)  => obj[ (mKey[mes] + base).toLowerCase() ] = Number(v)||0;
  ...
  function buildCapData(){
    ...
    const rowTOT= { Rubrica:"PRODUÇÃO TOTAL PRODUTOS ACABADOS", _total:true };
  ...
  function initGridCapacidade(){
    ...
    $("#gridCapacidade").dxDataGrid({
      ...
      columns: [
        { dataField:"Rubrica", caption:"Rubricas", allowEditing:false, width: 320,
          cellTemplate: (c, v) => {
        ...
        ...meses.map(m => ({
          dataField: m, caption: m, dataType:"number", format:"#0", alignment:"right",
          allowEditing: (r)=>!r.row.data._total,
          cssClass: "text-end"
        }))
      ],
      onCellPrepared: e => { if(e.data && e.data._total) e.cellElement.addClass('total-row'); },
      onRowUpdated: e => {
        // gravar nos campos do modelo
        meses.forEach(m=>{
          if (e.key.includes("Corte")) setMesVal(dados,"cc", m, e.data[m]);
          if (e.key.includes("Montagem")) setMesVal(dados,"ma", m, e.data[m]);
        });
  ...
  function buildProdData(){
    ...
    meses.forEach(m=>{
      ...
      cc[m]  = (c * d) || 0;
      ma[m]  = (a * d) || 0;
      tot[m] = ma[m]; // total de acabados = M&A × dias
    });
  ...
  function initGridProducao(){
    ...
    $("#gridProducao").dxDataGrid({
      ...
      columns: [
        { dataField:"Rubrica", caption:"Rubricas", width: 320,
          cellTemplate: (c,v)=>{ const el=$('<div>').text(v.value); if(v.data._total) el.addClass('total-row'); c.append(el); }
        },
        ...meses.map(m => ({ dataField:m, caption:m, dataType:"number", format:"#0", alignment:"right", cssClass:"text-end" })),
        { dataField:"ANO", caption:"ANO", dataType:"number", format:"#0", alignment:"right", cssClass:"text-end", width:110 }
      ],
      onCellPrepared: e => {
        if(e.data && (e.data._total || e.data._dias)) e.cellElement.addClass(e.data._total ? 'total-row' : '');
      }
    });
  }
  ...
  function initFormCustos(){
    $("#formCustos").dxForm({
      ...
      minColWidth: 220,
      items: [
        { dataField:"mediacc", label:{text:"Corte & Costura (C&C)"}, editorType:"dxNumberBox", editorOptions:{ min:0, format:"#0.00' €'", showSpinButtons:true } },
        { dataField:"mediama", label:{text:"Montagem & Acabamento (M&A)"}, editorType:"dxNumberBox", editorOptions:{ min:0, format:"#0.00' €'", showSpinButtons:true } },
      ],
  ...
  async function carregar(anoSel){
    ...
    if (r.status === 404){
      ...
      DevExpress.ui.notify("Sem registo para este ano. Cria um novo.", "warning", 2500);
    } else if(!r.ok){
    ...
    ano = anoSel; $("#anoInput").val(ano); touched=false; setSaveEnabled(false);
  ...
  async function guardar(){
    ...
    if(!resp.ok){ DevExpress.ui.notify(await resp.text() || "Erro a gravar.", "error", 3000); return; }
    dados = await resp.json(); touched=false; setSaveEnabled(false);
  ...
  async function criarAno(){
    ...
    const conf = await DevExpress.ui.dialog.confirm(
      `Criar produção de ${anoSel}? Se existir ${anoSel-1}, os valores serão copiados.`, "Criar ano"
    );
    if(!conf) return;
    ...
    if(!resp.ok){ DevExpress.ui.notify(await resp.text() || "Falha ao criar.", "error", 3000); return; }
